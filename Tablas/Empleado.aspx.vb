...
Public Class Empleado
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        If txtID.Text = "0" Or txtID.Text = "" Then
            Dim cmd As New MySqlCommand("INSERT INTO `empleado` (`NOMBRE`, `APELLIDO`, `TELEFONO`, `CORREO`, `ID_EMPRESA`, `ID_SUCURSAL`, `ACTIVO`) 
            ...
            Limpiar()
        Else
            Dim cmd As New MySqlCommand("UPDATE `empleado` SET `NOMBRE`='" & txtNombre.Text & "',`APELLIDO`='" & txtApellido.Text & "',`TELEFONO`='" & txtTelefono.Text & "',`CORREO`='" & txtCorreo.Text &
                                        "',`ID_EMPRESA`='" & ddlEmpresa.SelectedValue & "',`ID_SUCURSAL`= '" & ddlSucursal.SelectedValue & "',`ACTIVO`= '1' WHERE `ID_EMPLEADO`='" & txtID.Text & "'", Conexiones.Cnn)
            cmd.ExecuteNonQuery()
    ...
    Private Sub Poblar()
        ...
        Dim da As New MySqlDataAdapter("SELECT empleado.*, empresa.ID_EMPRESA AS 'EMPR_ID', empresa.NOMBRE AS 'EMPRESA', sucursal.ID_SUCURSAL AS 'SUCR_ID', sucursal.NOMBRE AS 'SUCURSAL' FROM `empleado` 
                                        ...
                                        WHERE empleado.NOMBRE LIKE '%" & txtBuscar.Text & "%' and empleado.ACTIVO = '1'", Conexiones.Cnn) 'SELECT sucursal.ID_SUCURSAL, empresa.NOMBRE as 'EMPRESA', sucursal.NOMBRE, sucursal.DIRECCION, sucursal.TELEFONO, sucursal.ACTIVO FROM sucursal INNER JOIN empresa on empresa.ID_EMPRESA = sucursal.ID_EMPRESA
        ' Dim ds As New DataSet
        dst = New DataSet
        da.Fill(dst)
        If dst.Tables(0).Rows.Count > 0 Then
            empleadoGrid.DataSource = dst.Tables(0)
            empleadoGrid.DataBind()

        Else
            empleadoGrid.DataSource = Nothing
            empleadoGrid.DataBind()
        End If
    ...
    Protected Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        If MsgBox("¿Seguro que desea eliminar este registro ?", vbYesNo + vbCritical + vbDefaultButton2, "Atención") = vbYes Then
            Dim cmd As New MySqlCommand("UPDATE `empleado` SET `ACTIVO`= 0 WHERE ID_EMPLEADO= " & txtID.Text, Conexiones.Cnn)
            cmd.ExecuteNonQuery()
    ...
    Protected Sub btnID_Click(sender As Object, e As EventArgs) Handles btnID.Click
        ...
        Dim da As New MySqlDataAdapter("SELECT empleado.*, empresa.ID_EMPRESA AS 'EMPR_ID', empresa.NOMBRE AS 'EMPRESA', sucursal.ID_SUCURSAL AS 'SUCR_ID', sucursal.NOMBRE AS 'SUCURSAL' FROM `empleado` 
                                        INNER JOIN empresa ON empresa.ID_EMPRESA = empleado.ID_EMPRESA
                                        INNER JOIN sucursal ON sucursal.ID_SUCURSAL=empleado.ID_SUCURSAL 
                                        WHERE empleado.ID_EMPLEADO ='" & txtID.Text & "' and empleado.ACTIVO = '1'", Conexiones.Cnn)
